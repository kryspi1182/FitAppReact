...
import { deleteUserSavedDiet, selectAllUserSavedDiets } from '../../../store/User/UserSavedDietsSlice';
...
const DeleteUserSavedDiets: React.FC = () => {
    ...
    return(<Container>
        ...
        {userSavedDiets.map(savedDiet => <Row className={classes.row}>
            <Col>
                {savedDiet.name}
            </Col>
            <Col>
                <Button type="button" variant="outlined" onClick={() => handleDelete(savedDiet.id)} color="secondary">Delete</Button>
            </Col>
        </Row>)}
    </Container>);
};
